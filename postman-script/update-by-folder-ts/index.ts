...
let localCollection: any = {}//JSON.parse(fs.readFileSync(`C:\\git\\api-specs\\postman\\collections\\sailpoint-api-v3.json`).toString())
...
const release = async () => {
    ...
    if (changesMade) {
        ...
        console.log('\n' + msg + '...')
        await new Collection(privateRemoteCollectionIdUid).merge(mainPublicCollectionId)
            .then(() => {
                console.log(msg, '-> OK\n')
            }
            )
...
async function refreshRemoteCollection(remoteCollectionID) {
    ...
    console.log('\n' + msg + '...\n')
    const remoteCollection = await new Collection(remoteCollectionID).get()
